        ...
        <>
          <div className="social-tasks">
            ...
            <div className="points-display">
              ...
              <p className="points-note">Note: Final verification will check actual social engagement (follows, posts, stars)</p>
            </div>
          </div>
                ...
                <div className="success-message">
                  ...
                  {!phase1.isActive && (
                    <div className="info-message" style={{ marginTop: '1rem' }}>
                      <p>⏳ Phase 1 is not active yet. Check back soon!</p>
                    </div>
                  )}

                  {phase1.isActive && !phase1.verified && !submitProof.isSuccess && (
                    <>
                      <button
                        onClick={() => submitProof.submit('@RiddlenToken', '@RiddlenCommunity')}
                        className="submit-blockchain-btn"
                        disabled={submitProof.isPending || submitProof.isConfirming}
                      >
                        {submitProof.isPending || submitProof.isConfirming ? '⏳ Submitting...' : `📡 Submit for ${eligibility.estimatedTokens.toLocaleString()} RDLN`}
                      </button>
                      <p className="note">Submit your eligibility to the smart contract for verification. You'll receive {eligibility.estimatedTokens.toLocaleString()} RDLN once verified.</p>
                    </>
                  )}

                  {submitProof.isSuccess && !phase1.verified && (
                    <div className="pending-verification">
                      <p>⏳ Awaiting operator verification...</p>
                      <a href={`https://amoy.polygonscan.com/tx/${submitProof.hash}`} target="_blank" rel="noopener noreferrer" className="tx-link">
                        View Transaction ↗
                      </a>
                    </div>
                  )}

                  {phase1.verified && !phase1.claimed && (
                    <>
                      <button
                      ...
                      >
                        {claimPhase1.isPending || claimPhase1.isConfirming ? '⏳ Claiming...' : `🎁 Claim ${eligibility.estimatedTokens.toLocaleString()} RDLN`}
                      </button>
                    </>
                  )}
